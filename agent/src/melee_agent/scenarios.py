...
KINDS = ("grounded", "airborne", "offstage", "ledge", "shielded")
...
class ScenarioV1:
    ...
    def __post_init__(self):
        if (self.schema_version != 1 or self.kind not in KINDS or type(self.direction) is not int or
                ...
                self.measurement_timeout_frames != 180):
...
SUITE = tuple(ScenarioV1(kind+"-"+("left" if direction < 0 else "right"), kind, direction)
    for kind in KINDS[:-1] for direction in (-1, 1)) + (ScenarioV1("shielded-opponent", "shielded", -1),)
...
def suite_hash():
    return hashlib.sha256(json.dumps([spec.manifest() for spec in SUITE], sort_keys=True).encode()).hexdigest()
...
class ScenarioPolicy:
    ...
    def report(self):
        return {"schema_version": 1, "scenario": self.spec.manifest(), "suite_sha256": suite_hash(),
...
def verified_trial(report, name):
    try:
        ...
        if (report["complete"] is not True or report["scenario"] != spec.manifest() or
                report["suite_sha256"] != suite_hash() or result["status"] not in RESULTS or
