#ifndef SB_MOVEMENT_TEST_GAME_H
#define SB_MOVEMENT_TEST_GAME_H

/* Independent typed host surface, not a PPC ABI mirror. No generic mocks. */
#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>
typedef uint8_t u8;
...
typedef struct { HSD_GObj* items; } HSD_GObjList;
...
/* Native controller.h masks; unsigned LR avoids the signed 1<<31 UB. */
enum {
    HSD_PAD_DPADUP = 0x8,
...
#define LINE_FLAG_PLATFORM (1 << 8) /* melee/mp/forward.h */
struct CpuFighter {
...
struct Fighter {
    ...
    struct {
        ...
        float jump_startup_time, ground_friction, walk_max_vel;
        float normal_landing_lag, hop_v_initial_velocity;
        float dash_max_velocity, jump_v_initial_velocity;
    } co_attrs;
    ...
    union {
        struct {
            union {
                ...
                struct { int x0; bool x4; float jump_mul; } jump;
                struct { int is_short_hop; ftCo_JumpInput jump_input; } kneebend;
                struct { Vec3 self_vel; int timer; } escapeair;
            };
        } co;
    } mv;
...
struct TestCommonData {
    float analog_shoulder_deadzone, escapeair_force, escapeair_decay;
    Vec2 escapeair_deadzone;
    float x340, x344, friction_when_above_walk_speed;
    float horizontal_stick_deadzone, vertical_stick_deadzone;
    int x334, xE4;
};
